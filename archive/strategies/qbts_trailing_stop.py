#!/usr/bin/env python3
"""
QBTS Trailing Stop Strategy
============================
Rules (identical logic to TSLA strategy, scaled for 100 shares):
  - Entry:         100 shares at market (already placed)
  - Stop Loss:     Sell all 100 shares if price drops 10% from fill price (floor never goes down)
  ...
  - Ladder In:     If live price drops 20% below fill price, buy 200 additional shares at market.
                   Monitored live each minute — not pre-placed (avoids Alpaca wash-trade block).

Run on a schedule every 1 minute during market hours via Windows Task Scheduler.
State is persisted to qbts_strategy_state.json so it survives restarts.
"""
...
STATE_FILE  = BASE_DIR / "strategies" / "qbts_strategy_state.json"
LOG_FILE    = BASE_DIR / "logs" / "qbts_strategy.log"
...
SYMBOL              = "QBTS"
INITIAL_QTY         = 100
LADDER_QTY          = 200   # 2x initial, same ratio as TSLA strategy
...
LADDER_DROP_PCT     = 0.20  # Buy 200 more shares if price drops 20% from fill

# Order ID for the initial market buy placed now
INITIAL_ORDER_ID = "66f6d980-953e-4174-b0e1-880887db71a9"
...
def load_state() -> dict:
    defaults = {
        ...
        "ladder_filled":   False,
        "position_closed": False,  # True once we no longer hold QBTS
    }
...
def update_stop(state: dict, new_stop_price: float, qty: int = None) -> dict:
    """Cancel existing stop and replace with new one at new_stop_price.
...
def place_ladder_market_buy(qty: int) -> dict:
    """Fire a market buy for the ladder when live price breaches -20%."""
    payload = {
        "symbol":        SYMBOL,
        "qty":           str(qty),
        "side":          "buy",
...
def run():
    log.info("=" * 65)
    log.info(f"QBTS Strategy Check  |  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log.info("=" * 65)
    ...
    if state["fill_price"] is None:
        ...
        if status == "filled":
            ...
            log.info(f"  Not filled yet. Will retry next run.")
            return
    ...
    # ── Step 2: Place initial stop loss (once) ────────────────────────────────
    ...
    # ── Step 3: Confirm position is open ─────────────────────────────────────
    position = get_position(SYMBOL)
    if position is None:
        log.info("No open QBTS position — likely stopped out. Strategy complete.")
        state["position_closed"] = True
        save_state(state)
        return

    held_qty = float(position.get("qty", 0))
    log.info(f"Position         : {held_qty:.0f} shares held")
    ...
    # ── Step 7: Live-price ladder trigger ─────────────────────────────────────
